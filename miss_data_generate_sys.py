...
# n = df[df.columns[1:]].apply(lambda x: x if random.randint(0, 100) > 40 else np.NaN)
res = pd.DataFrame(res)
df_res = pd.concat([df[df.columns[0]], res], axis=1)
df_res.columns = cols_name
print(df_res)
df_res.to_csv('dataelectricity/electricity_miss_10.csv', index=False)
